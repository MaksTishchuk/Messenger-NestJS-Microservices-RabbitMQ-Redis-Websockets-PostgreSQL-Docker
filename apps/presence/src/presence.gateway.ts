...
export class PresenceGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  // NOTE: For development, clear cache on restart server
  async onModuleInit() {
  ...
  private async emitStatusToFriends(activeUser: ActiveUser) {
    ...
    for (const f of friends) {
      const user = await this.cache.get(`user_${f.id}`)
      if (!user) continue
  ...
  private async setActiveStatus(socket: Socket, isActive: boolean) {
    ...
    await this.cache.set(`user_${user.id}`, activeUser)
  ...
  async handleConnection(socket: Socket) {
    ...
    // ob$ - signalize us about observable from rxjs
